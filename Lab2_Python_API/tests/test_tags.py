...
def multiple_tags(client):
    ...
    created = []
    for t in tags:
        resp = client.post("/tags", json=t)
        created.append(resp.json())
    return created


def test_get_tags_count(client, multiple_tags):
...
def test_get_tag_by_id(client):
    ...
    post = client.post("/tags", json=tag).json()

    response = client.get(f"/tags/{post['id']}")
    assert response.status_code == 200
...
def test_post_tag(client):
    ...
    get_all = client.get("/tags").json()
    assert len(get_all) == 1


def test_put_tag(client):
    tag = {"userId": 7, "movieId": 30, "tag": "Sci-fi", "timestamp": 2222222222}
    created = client.post("/tags", json=tag).json()

    update = {"tag": "Horror", "timestamp": 3333333333}
    response = client.put(f"/tags/{created['id']}", json=update)
    assert response.status_code == 200
...
def test_delete_tag(client):
    ...
    delete = client.delete(f"/tags/{created['id']}")
    assert delete.status_code == 200

    get = client.get(f"/tags/{created['id']}")
